...
GENE =args.gene
LOG_FILE_TRAIN_ACC = os.path.join(OUTPUT_FOLDER,'eval_train.csv')
LOG_FILE_VAL_ACC = os.path.join(OUTPUT_FOLDER,'eval_val.csv')
EXPERIMENT_TYPE="{}_{}_{}".format(GENE,CNN_TO_USE,pool_algorithm)
...
GENE_FOLDER = './'+OUTPUT_FOLDER.split('/')[1]+'/'+OUTPUT_FOLDER.split('/')[2]+'/'+OUTPUT_FOLDER.split('/')[3]+'/'
create_dir(GENE_FOLDER)
print('Created: ',GENE_FOLDER)
...
if (TASK=='binary'):
	...
	if (N_CLASSES==1):
		csv_filename_training = csv_folder+'{}_train.csv'.format(args.gene)
		csv_filename_validation = csv_folder+'{}_val.csv'.format(args.gene)
		print("training file: ",csv_filename_training)
...
class MIL_model(torch.nn.Module):
	def __init__(self):
		...
		if (EMBEDDING_bool==True):
			...
			self.mlp = torch.nn.Sequential(
				torch.nn.Linear(self.fc_feat_in, self.D),
				torch.nn.ReLU(),
				torch.nn.Linear(self.D, self.E),
				#torch.nn.ReLU(),
				#torch.nn.Linear(self.D, self.E),
			)
	...
	def forward(self, x, conv_layers_out, labels_wsi_np):
		...
		if (EMBEDDING_bool==True):
			...
			#embedding_layer = self.mlp(conv_layers_out)            
...
def evaluate_validation_set(val_file_log, model, epoch, generator):
...
log_train_acc = open(LOG_FILE_TRAIN_ACC, 'w')
log_val_acc = open(LOG_FILE_VAL_ACC, 'w')
log_train_acc.write('case name, prediction, label\n')
log_val_acc.write('case name, prediction, label\n')

print("+++++++++++++++++++++ EVAL VALIDATION SPLIT +++++++++++++++++++++++++++++++")
valid_loss, valid_wsi_store_loss, valid_patches_store_loss = evaluate_validation_set(log_val_acc, model, -1, validation_generator_bag)    
print("+++++++++++++++++++++ TRAINING +++++++++++++++++++++++++++++++")
while (epoch<num_epochs and early_stop_cont<EARLY_STOP_NUM):
	...
	for i in range(iterations):
		print('Epoch: [%d] (%d / %d) ' % (epoch, i, tot_batches_training))
		try:
		...
		for b in range(len_bag):
			...
			if (TASK=='geneExp'): 
				#csv_instances=[]
				#for b in inputs_bag[1]:     
				#file_path=os.path.join(WSI_FOLDER, b)
				#file_path=os.path.join(file_path, b+'_paths_densely.csv')
				if len(inputs_bag[1])<2:
					try:
						file_path_ts = os.path.join(WSI_FOLDER, inputs_bag[1][0])#, '_paths_densely.csv')
						file_path_bs = None
			...
			with torch.no_grad():
				for instances in training_generator_instance:
					...
					feats_np = feats.cpu().data.numpy()
					features = np.append(features,feats_np)
					#print("[debug] features: {}".format(features))
				#del instances
			...
			loss = criterion_wsi(predictions, labels)
			print("[debug] loss: {}".format(loss))
			loss.backward() 

			outputs_wsi_np = predictions.cpu().data.numpy()
			print("[debug] predictions: {}".format(outputs_wsi_np))
			...
			print("[debug] wsi_store_loss: {}".format(wsi_store_loss))
			...
			if (N_CLASSES==5):
				...
				y_pred = np.append(y_pred, outputs_wsi_np)
				y_true = np.append(y_true,labels_wsi_np)
				filenames_wsis = np.append(filenames_wsis,filename_wsi)
				pred_cancers = np.append(pred_cancers,outputs_wsi_np)
				if TASK=='binary':
					...
					micro_accuracy_train = MSE(y_true, y_pred)
					print("micro MSE " + str(micro_accuracy_train))
	...
	print("+++++++++++++++++++++ EVAL VALIDATION SPLIT +++++++++++++++++++++++++++++++")
	valid_loss, valid_wsi_store_loss, valid_patches_store_loss = evaluate_validation_set(log_val_acc, model, epoch, validation_generator_bag)
	#save validation
...
"""
	...
	print("[debug] best_loss: {}, valid_loss: {}".format(best_loss, valid_loss))
